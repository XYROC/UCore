	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (args.length == 1) {
			...
			if (args[0].equalsIgnoreCase("help")) {
				...
				sender.sendMessage(ChatColor.GOLD + "/uc chest" + ChatColor.GRAY
						+ ": Generiert eine Kiste mit zufälligen Items. (Spieler-Kommando) (Fun)");
				...
				sender.sendMessage(ChatColor.GOLD + "/uc path" + ChatColor.GRAY
						+ ": Zeigt dir den Pfad des laufenden Minecraft-Servers");
			...
			if (args[0].equalsIgnoreCase("config")) {
				if (args.length == 2) {
					if (args[1].equalsIgnoreCase("help")) {
						sender.sendMessage(ChatColor.GOLD + "/uc config" + ChatColor.GRAY + ": Config-Hauptkommando");
						sender.sendMessage(ChatColor.GOLD + "/uc config setDefaultServerRunScript <file>"
								+ ChatColor.GRAY + ": Setzt die Scriptdatei, die den Server startet.");
					}
				}
				if (args.length == 3) {
					if (args[1].equalsIgnoreCase("setDefaultServerRunScript")) {
						...
						try {
							config.save(file);
							sender.sendMessage(ChatColor.GRAY + "Der Server wird nun automatisch mit " + ChatColor.GREEN
									+ "'" + args[2] + "'" + ChatColor.GRAY + " neu gestartet.");
						} catch (IOException e) {
							e.printStackTrace();
						}
